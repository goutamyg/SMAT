...
class LinearSelfAttention(BaseLayer):
    ...
    def visualize_context_scores(context_scores):
        ...
        try:
            ...
            context_map = cv2.resize(
                context_map, (256, 256), interpolation=cv2.INTER_NEAREST
            )
    ...
    def _forward_self_attn(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        """
        if context_scores.shape[3] == 320:
            self.visualize_context_scores(context_scores=context_scores[:, :, :, 0:256])
        elif context_scores.shape[3] == 80:
            self.visualize_context_scores(context_scores=context_scores[:, :, :, 0:64])
        """

...
def initialize_weights(opts, modules) -> None:
    ...
    conv_init_type = getattr(opts, "conv_init", "kaiming_normal")
    linear_init_type = getattr(opts, "linear_init", "normal")

    conv_std = getattr(opts, "conv_init_std_dev", None)
    linear_std = getattr(opts, "linear_init_std_dev", 0.01)
    group_linear_std = getattr(opts, "group_linear_init_std_dev", 0.01)

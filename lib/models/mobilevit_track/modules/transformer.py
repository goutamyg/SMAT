...
class LinearAttnFFN(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pre_norm_attn = nn.Sequential(
            get_normalization_layer(opts=opts, norm_type=norm_layer, num_features=embed_dim),
            attn_unit,
            Dropout(p=dropout),
        )
    ...
    def forward(
        ...
        if x_prev is None:
            ...
            # cross-attention
            res = x
            x = self.pre_norm_attn[0](x)  # normalize x
            x_prev = self.pre_norm_attn[0](x_prev)  # normalize x_prev
            x = self.pre_norm_attn[1](x, x_prev)  # attn
            x = self.pre_norm_attn[2](x)  # dropout
            x = x + res  # residual

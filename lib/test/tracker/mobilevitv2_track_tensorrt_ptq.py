...
class MobileViTv2Track(BaseTracker):
    ...
    def __init__(self, params, dataset_name):
        ...
        self.quant_onnx_file_path = Path(weights_path.with_suffix('.nncf.onnx'))
        self.engine_file_path = Path(weights_path.with_suffix('.nncf.engine'))

        # Specify model precision
        self.precision = np.float32

        if not os.path.exists(self.quant_onnx_file_path):
            print("Quantizing onnx model now....This may take a while")
            # Perform and calibration and quantization of the onnx model
            self.quant_onnx_model = self.quantization()
        else:
            print(f"Model exists! Reading model from {self.quant_onnx_file_path}")
            self.quant_onnx_model = onnx.load(self.quant_onnx_file_path)
        ...
        self.engine = self.get_engine(self.quant_onnx_file_path, self.engine_file_path)

       # Check the precision mode of the engine
        precision_mode = "INT8" if self.engine.has_implicit_batch_dimension and self.engine.precision == trt.DataType.INT8 else "FP32"
        print(f"The TensorRT engine is in {precision_mode} precision mode.")
    ...
    def quantization(self):
        """
        Method that first calibrates the fp32 onnx model to determine the range for weight
        and activation quantization, and then subsequently quantizes the model into its
        int8 equivalent.
        """

        # Suppress NNCF warnings
        warnings.filterwarnings("ignore", category=UserWarning, module="nncf")

        input_name_1 = self.onnx_model.graph.input[0].name
        input_name_2 = self.onnx_model.graph.input[1].name

        def transform_fn(data_item):
            z, x  = data_item
            return {input_name_1: z, input_name_2: x}
        ...
        print("Calibrating now...This may take a while.")
        calibration_dataset = nncf.Dataset(inputs, transform_fn)
        ...
        print(f"Saving model to {self.quant_onnx_path}")
        onnx.save(torch_quantized_model, self.quant_onnx_path)
        print("Model saved successfully!")

        return torch_quantized_model
    ...
    def build_engine(self, onnx_file_path, engine_file_path):
        ...
        with trt.Builder(self.TRT_LOGGER) as builder, builder.create_network(1 << int(trt.NetworkDefinitionCreationFlag.EXPLICIT_BATCH)) as network:
            ...
            with trt.Runtime(self.TRT_LOGGER) as runtime:
                pdb.set_trace()  # Set a breakpoint here, this is where the error originates at
                plan = builder.build_serialized_network(network, config)
                if plan is None:
                    raise RuntimeError("Failed to build serialized engine. Check if there are any errors during build.")

...
warnings.filterwarnings("ignore", category=UserWarning, module="torch_tensorrt")
...
class MobileViTv2Track(BaseTracker):
    ...
    def __init__(self, params, dataset_name):
        ...
        # Path where PyTorch model is saved 
        weights_path = Path(self.params.checkpoint.split('.tar')[0])

        # Path where TensorRT torchscrpt module is stored.
        self.trt_path = Path(weights_path.with_suffix('.ps'))

        # Print tensorrt information to console
        # torch_tensorrt.dump_build_info()

        #  Optimize and compile PyTorch model to TensorRT   
        self.trt_model = self.compile_model(self.trt_path)
        self.trt_model.eval().to(self.device)


    def compile_model(self, trt_path):
        if os.path.exists(trt_path):
            print("Optimized model exists.....loading from {}".format(trt_path))

            with torch_tensorrt.logging.debug():
                trt_module = torch.jit.load(trt_path, map_location=self.device)
            print("Model loaded successfully!")
            return trt_modules
        else:
            print("Compiling model to optimized TensorRT engine")
                        
            # Define inputs to the model
            dummy_input = (torch.randn([1,128,32,32], dtype=torch.float32).to(self.device),
            torch.randn([1,3,256,256], dtype=torch.float32).to(self.device))

            input_signature = ((torch_tensorrt.Input(shape=[1, 128, 32, 32], dtype=torch.float32),
                         torch_tensorrt.Input(shape=[1, 3, 256, 256], dtype=torch.float32))
                         )
            # Specify precisions
            enabled_precisions = {torch.float32}

            # Trace model 
            traced_model = torch.jit.trace(self.network, dummy_input, strict=True)
            # print(traced_model.graph)
            # Access output names
            print(["output_{}".format(i) for i in range(len(traced_model(*dummy_input)))])
            
            with torch_tensorrt.logging.errors():
                # Compile model 
                trt_module = torch_tensorrt.compile(traced_model, input_signature=input_signature, 
                    enabled_precisions=enabled_precisions, truncate_long_and_double=True,
                    workspace_size= 1 << 30)

            print("Saving model to {}".format(trt_path))
            torch.jit.save(trt_module, self.trt_path)
            return trt_module
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            inputs = (self.z_dict1.to(self.device), x_dict.tensors.to(self.device))

            out_dict = self.trt_model(*inputs)

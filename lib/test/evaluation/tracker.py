...
class Tracker:
    ...
    def run_video(self, videofilepath, optional_box=None, debug=None, visdom_info=None, save_results=False):
        ...
        if os.path.isfile(videofilepath):
            ...
            import glob
            list_imgs = sorted(glob.glob(videofilepath + '/img/*.jpg'))
        else:
        ...
        while True:
            ...
            cv.imwrite("{}.png".format(f_name), np.concatenate((out["search_area"], out["target_response"]), axis=1))

            # Save the search area with the bounding box results
            video_name = videofilepath.split('/')[-1]
            res_dir_name = os.path.join("vis_search_area", video_name)
            if not os.path.isdir(res_dir_name):
                os.makedirs(res_dir_name)
            f_name = "{}/index_{}".format(res_dir_name, list_imgs[frame_idx-1].split('/')[-1].split('.')[0])
